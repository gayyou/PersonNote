...
- 动态属性：
  ...
  以`v-bind:key`或者`:key`开头，并且`key`是常量。

  - 静态属性名：对于静态属性名的情况下，通过正则表达对属性进行获取，属性值以字符串进行存储，以便在创建渲染函数的时候形成变量从而进行访问。
  - 动态属性名：动态属性名再添加的时候，会标注上属性名是动态的。同样属性值也会用字符串进行存储。
...
对于标签内部的文本处理，最终目的是提取文本里面的变量和常量，组成一个渲染函数放到抽象语法树的节点上面。处理常量和变量的策略还是依旧利用`JSON.stringify`这个来解析常量作为渲染函数的字符串常量，通过普通字符串来解析变量，作为渲染函数的代码。

### 3.处理结果

处理的结果是将句法分析器分析的结果进行属性的提取分类、生成渲染函数。最终封装到抽象语法树的节点中，以便接下来进行生成VNode的时候使用。